...
### How do I update DClaw Recruit?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp recruit --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Recruit without Kubernetes?

Yes, for local development:

```bash
cd dclaw-recruit/frontend && npm run dev
cd dclaw-recruit/backend && uvicorn dclaw_recruit.main:app --reload
```
